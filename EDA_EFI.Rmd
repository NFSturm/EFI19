...
To model the findings, we will use three distinct methods: Linear regression, Bayesian regression and a random forest for regression. First of all, the data is split into training and test sets. 

```{r}
splits <- initial_split(efi19_d, prop = 4/5)
train <- training(splits)
test <- testing(splits)
```

To enable the modeller to estimate an adequate Bayesian model, we shall try to infer a good prior distribution.

```{r echo = FALSE}
bayes <- stan_glm(gdp_percap ~ ., data = train, chains = 3, warmup = 300, iter = 1800)
```

```{r message = FALSE}
actual <- test$gdp_percap
stan_pred <- posterior_predict(bayes, test, draws = 1)
rmse(actual, stan_pred)
```

```{r}
control <- trainControl(method = 'repeatedcv', number = 10, repeats = 3, search = 'grid')
rf_model <- train(gdp_percap ~., data = train, method = "ranger", trainControl = control)
```
